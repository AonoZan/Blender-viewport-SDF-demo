...
bvh_leaf_prims_height = -1  # Tracks height of leaf prims texture in Python
...
def create_many_sdf_empties(num_to_create):
    print(f"Creating/updating {num_to_create} empties...")
    ...
    for i in range(gs):
        ...
        for j in range(gs):
            ...
            c += 1
    print(f"Done: Created/updated {c} empties.")
...
# --- Shaders for Blender 5.0 (Declarations handled by GPUShaderCreateInfo) ---
vertex_shader = '''
void main() {
...
fragment_shader = '''
mat4 get_mat(int si, int btx) {
    vec4 c0 = texelFetch(shapeDataTex, ivec2(btx, si), 0);
    vec4 c1 = texelFetch(shapeDataTex, ivec2(btx + 1, si), 0);
    vec4 c2 = texelFetch(shapeDataTex, ivec2(btx + 2, si), 0);
    vec4 c3 = texelFetch(shapeDataTex, ivec2(btx + 3, si), 0);
    return mat4(c0, c1, c2, c3);
}

float s_sph(vec3 p, mat4 wm, float r) {
    mat4 im = inverse(wm);
    vec3 pl = (im * vec4(p, 1.0)).xyz;
    vec3 s = vec3(length(wm[0].xyz), length(wm[1].xyz), length(wm[2].xyz));
    float ms = max(0.0001, min(s.x, min(s.y, s.z)));
    pl /= ms;
    return (length(pl) - r) * ms;
}

float s_box(vec3 p, mat4 wm, vec3 b) {
    mat4 im = inverse(wm);
    vec3 pl = (im * vec4(p, 1.0)).xyz;
    vec3 s = vec3(length(wm[0].xyz), length(wm[1].xyz), length(wm[2].xyz));
    float ms = max(0.0001, min(s.x, min(s.y, s.z)));
    pl /= ms;
    vec3 q = abs(pl) - b;
    float dl = length(max(q, vec3(0.0))) + min(max(q.x, max(q.y, q.z)), 0.0);
    return dl * ms;
}

float s_tor(vec3 p, mat4 wm, vec2 t) {
    mat4 im = inverse(wm);
    vec3 pl = (im * vec4(p, 1.0)).xyz;
    vec3 s = vec3(length(wm[0].xyz), length(wm[1].xyz), length(wm[2].xyz));
    float ms = max(0.0001, min(s.x, min(s.y, s.z)));
    ...
    float dl = length(q) - t.y;
    return dl * ms;
}

float s_cyl(vec3 p, mat4 wm, vec2 h) {
    mat4 im = inverse(wm);
    vec3 pl = (im * vec4(p, 1.0)).xyz;
    vec3 s = vec3(length(wm[0].xyz), length(wm[1].xyz), length(wm[2].xyz));
    float ms = max(0.0001, min(s.x, min(s.y, s.z)));
    pl /= ms;
    vec2 da = abs(vec2(length(pl.xz), pl.y)) - h;
    float dl = min(max(da.x, da.y), 0.0) + length(max(da, vec2(0.0)));
    return dl * ms;
}
...
bool intersectRayAABB(vec3 ro, vec3 invRd, vec3 bmin, vec3 bmax, out float ten, out float tex) {
    ...
    vec3 tmn = min(t1, t2);
    vec3 tmx = max(t1, t2);
    ten = max(tmn.x, max(tmn.y, tmn.z));
    tex = min(tmx.x, min(tmx.y, tmx.z));
...
float sdf_bvh(vec3 prp, vec3 ray_origin_world, vec3 ray_direction_world, vec3 inv_ray_dir) {
    ...
    float t_entry_child1, t_exit_child1;
    float t_entry_child2, t_exit_child2;
    ...
    while (stack_ptr > 0) {
        ...
        if (is_leaf_flag > 0.5) {
            ...
            int left_child_idx = int(node_meta.r);
            int right_child_idx = int(node_meta.g);
            bool hit_left = false;
            ...
            if (hit_left && hit_right) {
                if (t_entry_child1 < t_entry_child2) {
                    if (stack_ptr < 64) stack[stack_ptr++] = right_child_idx;
                    if (stack_ptr < 64) stack[stack_ptr++] = left_child_idx;
                } else {
                    if (stack_ptr < 64) stack[stack_ptr++] = left_child_idx;
                    if (stack_ptr < 64) stack[stack_ptr++] = right_child_idx;
                }
...
void main() {
    vec2 suv = gl_FragCoord.xy / viewportSize;
    vec2 ndc = suv * 2.0 - 1.0;
    vec3 ro;
    vec3 rd;
    if (isPerspective) {
        ro = cameraPos_world;
        vec4 fc = vec4(ndc.x, ndc.y, 1.0, 1.0);
        vec4 fw = inv_vpm * fc;
        rd = normalize(fw.xyz / fw.w - ro);
    } else {
        vec4 nc = vec4(ndc.x, ndc.y, -1.0, 1.0);
        vec4 nw = inv_vpm * nc;
        ro = nw.xyz / nw.w;
        rd = normalize((inv_vm * vec4(0.0, 0.0, -1.0, 0.0)).xyz);
    }
    ...
    for (int i = 0; i < MRS; ++i) {
        ...
        if (abs(d) < HE) {
            ...
            vec4 clp = vpm * vec4(cp, 1.0);
            float dp = clp.z / clp.w;
            gl_FragDepth = (dp * 0.5 + 0.5);
...
def upd_sh_tex(objs):
    ...
    blen = len(dlist)
    try:
        # Recreate GPU buffer every frame to force update
        shape_texture_buffer = gpu.types.Buffer('FLOAT', blen)
        shape_texture_buffer[:] = dlist
    except Exception as e:
        print(f"Error creating Shape Buffer: {e}")
        return None, 0

    tdims = (SHAPE_TEXTURE_WIDTH, MAX_TEXTURE_SHAPES)
    try:
        # Recreate texture every frame so GPU receives fresh transformation matrices
        ...
        print(f"Error creating Shape Texture: {e}")
        return None, 0
...
def upd_bvh_tex(flat_bvh_node_list, all_leaf_primitive_indices_flat):
    global bvh_aabb_texture, bvh_aabb_buffer, bvh_meta_texture, bvh_meta_buffer, \
           bvh_leaf_prims_texture, bvh_leaf_prims_buffer, bvh_leaf_prims_height
    ...
    try:
        ...
        print(f"Error creating BVH AABB Buffer: {e}")
        return False
    ...
    try:
        ...
        print(f"Error creating BVH AABB Texture: {e}")
        return False

    # 2. Meta Buffer & Texture
    meta_w = 1
    ...
    try:
        ...
        print(f"Error creating BVH Meta Buffer: {e}")
        return False
    ...
    try:
        ...
        print(f"Error creating BVH Meta Texture: {e}")
        return False

    # 3. Leaf Primitives Buffer & Texture
    if not all_leaf_primitive_indices_flat:
        bvh_leaf_prims_texture = None
        bvh_leaf_prims_height = -1
    else:
        ...
        try:
            ...
            print(f"Error creating BVH Leaf Prims Buffer: {e}")
            return False
        ...
        try:
            ...
            bvh_leaf_prims_height = texture_height
            if hasattr(bvh_leaf_prims_texture, 'interpolation'):
                bvh_leaf_prims_texture.interpolation = 'Closest'
        except Exception as e:
            print(f"Error creating BVH Leaf Prims Texture: {e}")
            bvh_leaf_prims_height = -1
            return False
    return True


# --- Uniform Safe Binding Helpers ---
...
def draw_cb():
    ...
    safe_uniform_float(shader_object, "cameraPos_world", cam_pos_w_val)
    safe_uniform_float(shader_object, "maxDist", max_d_val)
    safe_uniform_bool(shader_object, "isPerspective", is_p)
    safe_uniform_float(shader_object, "vpm", vpmat_val)
    safe_uniform_float(shader_object, "inv_vpm", inv_vpmat_val)
    safe_uniform_float(shader_object, "inv_vm", inv_vmat_val)
    safe_uniform_float(shader_object, "viewportSize", vpsize_val)
    safe_uniform_sampler(shader_object, "shapeDataTex", tex_s)
    safe_uniform_sampler(shader_object, "bvhABBTex", bvh_aabb_texture)
    safe_uniform_sampler(shader_object, "bvhMetaTex", bvh_meta_texture)
    if bvh_leaf_prims_texture:
        safe_uniform_sampler(shader_object, "bvhLeafPrimsTex", bvh_leaf_prims_texture)
    safe_uniform_int(shader_object, "num_bvh_nodes", num_bvh_nodes_val)
    safe_uniform_int(shader_object, "numActiveShapesInTex", num_s)
    safe_uniform_float(shader_object, "rimCenterBalance", rc_balance)
    safe_uniform_float(shader_object, "rimSharpness", rc_sharpness)
...
class SDF_PT_RaymarchSettingsPanel(bpy.types.Panel):
    bl_label = "SDF Raymarcher FX (BVH)"
...
def register_addon():
    ...
    try:
        ...
        interface = gpu.types.GPUStageInterfaceInfo("bvh_interface")
        ...
        info.push_constant('MAT4', "vpm")
        info.push_constant('MAT4', "inv_vpm")
        info.push_constant('MAT4', "inv_vm")
        info.push_constant('VEC2', "viewportSize")
        info.push_constant('INT', "num_bvh_nodes")
        info.push_constant('INT', "numActiveShapesInTex")
        info.push_constant('FLOAT', "rimCenterBalance")
        info.push_constant('FLOAT', "rimSharpness")
        
        info.sampler(0, 'FLOAT_2D', "shapeDataTex")
        info.sampler(1, 'FLOAT_2D', "bvhABBTex")
        info.sampler(2, 'FLOAT_2D', "bvhMetaTex")
        info.sampler(3, 'FLOAT_2D', "bvhLeafPrimsTex")
        
        info.vertex_source(vertex_shader)
        info.fragment_source(fragment_shader)
        
        shader_object = gpu.shader.create_from_info(info)
        print("Shader compiled successfully with GPUShaderCreateInfo.")
    except Exception as e:
        print(f"Shader compile failed: {e}")
        shader_object = None
        return
    ...
    print("Draw callback registered.")
    if scene_upd_hdl not in bpy.app.handlers.depsgraph_update_post:
        ...
        print("Depsgraph handler registered.")
